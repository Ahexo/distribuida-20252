class Diametro:
    ...
    def colapsar_lista(lista_1: dict, lista_2: dict):
        return lista_1

    def start_diametro(self):
        self.log(
            "Iniciando algoritmo para calcular la distancia a todos los procesos alcanzables."
        )
        ...
        for vecino in self.vecinos:
            vecino.msg(
                "go_consultar_diametro", (self, self.vecinos_inmediatos.copy()), self
            )
    ...
    def go_consultar_diametro(self, origen, conocidos, remitente):
        ...
        if (origen not in self.referidos) or (
            conocidos[remitente] < conocidos[self.referidos[origen]]
        ):
            self.referidos[origen] = remitente
            self.log(f"Lista de referidos actualizada: {self.referidos}")
        else:
            self.log(
                f"Este proceso ya recibió antes un mensaje por parte de {self.referidos[origen]} preguntando por las distancias de {origen}."
            )
        ...
        if len(diferencia_local) == 0:
            self.log(
                f"No hay procesos que aportar a la lista de distancias de {origen}, mandando back_consultar_diametro()"
            )
            self.log(f"Lista final por devolver: {conocidos}")
            remitente.msg("back_consultar_diametro", (origen, conocidos), self)
        else:
            self.log(
                f"Este proceso conoce {len(diferencia_local)} que aún no está(n) en la lista de {origen} que recibió de {remitente}"
            )
            ...
            self.log(
                f"Se han añadido {diferencia_local} a la lista de {origen} con distancia {distancia_de_los_procesos_a_referir}"
            )
            self.log(f"Lista actualizada: {conocidos}")
            self.log(
                f"Mandando go_consultar_diametro() con la lista actualizada de {origen} a {diferencia_local}"
            )
