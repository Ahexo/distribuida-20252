...
class nodo:
    ...
    def go_dfs(self, visited: set, remitente):
        ...
        if self.vecinos.issubset(visited):
            print(
                f"[RX] El conjunto de vecinos de {self} es subconjunto del de visitados."
            )
            visited.add(self)
            print(
                f"[RX] Mandando BACK({visited}, {self}) y vaciando el conjunto de hijos."
            )
            ...
            print(
                f"[RX] La diferencia de nodos entre visitados y vecinos de {self} es {dif}"
            )
            k = next(iter(dif))
    ...
    def go_bfs(self, distancia: int, remitente):
        if self.bfs_parent is None:
            ...
            print(
                f"[RX BFS] {self}: Padre: {self.bfs_parent}, Hijos: {self.bfs_children}, Nivel: {self.bfs_level}, ExpMsg: {self.expected_msg}"
            )
            if self.expected_msg == 0:
                print(
                    f"[RX BFS] {self} no tiene más vecinos a los que mandar mensajes, mandando BACK(Yes, {self.bfs_level}) al padre {self.bfs_parent}"
                )
                ...
                print(
                    f"[RX BFS] {self} está enviando mensajes GO({self.bfs_level}) a sus vecinos: {destinatarios}"
                )
                for k in destinatarios:
                    k.go_bfs(distancia + 1, self)
        elif self.bfs_level > distancia + 1:
            print(
                f"[RX BFS] {self} ha encontrado que si es hijo de {remitente} su nivel mejorará de {self.bfs_level} a {distancia + 1}"
            )
            self.refresh_bfs(distancia, remitente)
            print(
                f"[RX BFS] {self}: Padre: {self.bfs_parent}, Hijos: {self.bfs_children}, Nivel: {self.bfs_level}, ExpMsg: {self.expected_msg}"
            ...
            print(
                f"[RX BFS] {self} (nivel {self.bfs_level}) ha recibido una oferta de paternidad de {remitente} a {distancia + 1}, pero su nivel no mejoraría."
            )
    ...
    def back_bfs(self, cambiar_padre: bool, distancia: int, remitente):
        print(
            f"[RX BFS] {self} recibió un mensaje BACK de {remitente} con estado ({cambiar_padre}, {distancia})"
        )
        print(
            f"[RX BFS] Distancia recibida de {remitente}: {distancia}. Nivel de {self}: {self.bfs_level}; "
        )
        ...
        if distancia == self.bfs_level + 1:
            ...
            if cambiar_padre is True:
                ...
                print(
                    f"[RX BFS] {remitente} ha sido anexado en la lista de hijos de {self}"
                )
                # self.expected_msg -= 1
                print(
                    f"[RX BFS] {self}: Padre: {self.bfs_parent}, Hijos: {self.bfs_children}, Nivel: {self.bfs_level}, ExpMsg: {self.expected_msg}"
                ...
                if self.expected_msg <= 0:
                    if self.bfs_parent is self:
                        ...
                        print(
                            f"[RX BFS] El nodo {self} ha terminado de computar. Mandando BACK(Yes, {self.bfs_level}) a su padre {self.bfs_parent}."
                        )
        else:
            print(
                f"{self} está enterado de que {remitente} rechazó su oferta de paternidad"
            )
